...
public class WebSession implements Serializable {
    ...
    public void approbationLoginRedirect() throws IOException {
        FacesContext fc = FacesContext.getCurrentInstance();
        ExternalContext ext = fc.getExternalContext();
        if (!isSignedIn()) {
            ext.redirect(APPROBATION_LOGIN_REDIRECT_URL);
        }
    }
    
    public void approbationGuestRedirect() throws IOException {
        FacesContext fc = FacesContext.getCurrentInstance();
        ExternalContext ext = fc.getExternalContext();
        if (isSignedIn()) {
            ext.redirect(APPROBATION_INDEX_URL);
        }
    }
