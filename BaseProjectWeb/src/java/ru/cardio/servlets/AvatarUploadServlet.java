 ...
 * @author rogvold
 */
@WebServlet(name = "AvatarUploadServlet", urlPatterns = {"/upload_avatar"})
public class AvatarUploadServlet extends HttpServlet {

    private static final Logger log = Logger.getLogger(ru.reshaka.web.servlets.FileUploadServlet.class);
...
//            log.debug("doPost(): unauthorized attempt to upload file(s).");
        ...
        System.out.println("sabir is " + request.getParameter("sabir"));
        
        Object o = request.getAttribute(FILE_PARAM);
        System.out.println("attribute files_param = " + o);
...
//            log.debug("doPost(): error occured whule uploading files.");
        ...
        try {
            ...
            processFiles(u.getId(), fileItems, false, out);
        } catch (Exception ex) {
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            System.out.println("");
...
//                Attachment attachment = am.uploadFiles(u, Collections.singletonList(uf), null);
                Attachment attachment = am.uploadAvatar(uf.getFileName(), uf.getContentType(), u, uf.getContents(), 100, 100);
